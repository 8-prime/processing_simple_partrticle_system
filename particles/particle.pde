class Particle {
  ...
  Particle(float x, float y) {
    this.velX = random(-10, -40);//Random speed on the X axis for spread of particles
    this.velY = random(-2, 2);//Random speed on the Y axis 
    this.x = x;
    this.y = y;
    this.size = random(5, 10);//Random size of the particle
    col = color(random(150, 255), random(20, 100), random(20, 100));//random color
  }
